        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header">
                    <h3 class="card-title">ویرایش مقدار فرم کالا ({{ $categoryAttribute->name }})</h3>
                ...
                <div class="card-body">
                    ...
                    <form
                        action="{{ route('admin.market.property.value.update', ['categoryAttribute' => $categoryAttribute->id, 'categoryValue' => $categoryValue->id]) }}"
                        method="post">
                        @csrf
                        @method('put')
                        <div class="form-group">
                            ...
                            <select name="product_id" id="" class="form-control form-control-sm">
                                ...
                                @foreach ($categoryAttribute->category->products as $product)
                                    <option value="{{ $product->id }}" @if (old('product_id', $categoryValue->product_id) == $product->id) selected @endif>
                                        {{ $product->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="price_increase"
                                value="{{ old('price_increase', json_decode($categoryValue->value)->price_increase) }}"
                                class="form-control form-control-sm">
