                                ...
                                @if ($product->CategoryValues()->count() > 0)
                                    ...
                                    <form action="{{ route('front.market.add-review', $product) }}" method="post">
                                        <div class="lg:flex flex-wrap gap-y-5 justify-center">
                                            ...
                                            <div>
                                                <button
                                                    class="inline-block px-8 py-2 my-1 font-semibold leading-normal text-center text-white bg-red-500 align-middle transition-all ease-in border-0 rounded-lg shadow-md cursor-pointer text-xs active:opacity-85"
                                    ...
                                    @if ($averageScore->count() > 0)
                                        <div style="width: 100%;height:1px;background:dodgerblue;margin-top:4px"></div>
                                        <div class="md:px-10 mb-3 mt-5">
                                            <div class="opacity-80 mb-2">
                                                نمرات ثبت شده توسط کاربران :
                                            </div>
                                            @foreach ($averageScore as $i)
                                                @php
                                                    ...
                                                    $countsOfReviews = App\Models\Market\ProductReview::where('product_id', $product->id)
                                                        ->where('category_attribute_id', $i->category_attribute_id)
                                                        ->count();
                                                @endphp
                                                <div class="opacity-75 text-sm mt-4" style="width:145px;float:right">
                                                    ...
                                                    <p>
                                                        {{ number_format($i->average_score, 1, '.', '،') }} از
                                                        {{ $countsOfReviews }}
                                                        بررسی
                                                    </p>
                                                </div>
