...
<div class="max-w-[1440px] mx-auto px-3">
    <div class="bg-white shadow-xl my-5 lg:my-10 rounded-xl md:rounded-2xl p-3 md:p-5">
      <div class="mb-2">
        <div class="text-xl text-green-700 opacity-90 mb-3">
          سفارش با موفقیت انجام شد
        </div>
        <div class="text-sm opacity-70">
          پرداخت با موفقیت انجام شد. از انتخاب شما سپاسگزاریم.
        </div>
      </div>
                ...
                @foreach ($order->orderItems as $item)
                        <tr class="bg-white border-b hover:bg-gray-50 grid grid-cols-1 justify-items-center md:table-row">
                            ...
                            <td class="px-6 py-4 text-sm opacity-90 text-gray-900">
                                {{priceFormat(($item->singleProduct->price)+$item->color->price_increase+$item->guarantee->amount)}}
                                تومان
                            </td>
                        </tr>
                        @endforeach
    ...
    <div class="border shadow-xl rounded-2xl mx-auto max-w-xl mt-7 flex flex-col gap-y-5 py-5 px-5 md:px-20">
        <div class="flex justify-between">
            <div>
                قیمت کالاها ({{$order->orderItems->count()}}):
            </div>
        ...
        <div class="flex justify-between">
            ...
            <div class="flex gap-x-1">
                <div id="total_price">
                    {{priceFormat(($order->order_final_amount) - ($order->order_total_products_discount_amount)) }}
                </div>
                <div>
                    تومان
                </div>
      ...
      <span class="flex justify-center items-center opacity-90 my-5">
        <a href="{{route('front.home')}}" class="px-7 py-2 text-center text-white bg-red-500 align-middle border-0 rounded-lg shadow-md text-xs">صفحه اصلی</a>
      </span>
    </div>
  </div>
@endsection